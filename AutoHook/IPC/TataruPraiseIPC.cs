...
/// </para>
/// <para>
/// 🔴 <b>只能從 Framework 執行緒呼叫。</b>IPC 的實作跑在<b>呼叫端</b>的執行緒上，從背景 Task 叫過去
/// 等於把對方的程式碼拉到背景執行緒。唯一的呼叫點是 <c>FishingManager.FlushPendingPraises</c>，
/// 它掛在 <c>OnFrameworkUpdate</c> 的最開頭。
/// <br/>🔴 <b>刻意不從 <c>UpdateCatchDetour</c> 直接呼叫</b>（改動前是那樣）：detour 是我們 hook 住
/// 遊戲函式的那一格堆疊，在裡面做跨外掛呼叫等於把對方的碼拉進遊戲處理釣獲的中途——
/// 它慢一點就是遊戲卡一下，它擲例外就是從遊戲的呼叫框架裡往上竄。detour 那一側現在只入列。
/// </para>
/// <para>
